...
/** The role hierarchy: super_admin > admin > user(assistant). */
...
export interface CurrentUser {
  ...
  username: string;
  id: UUID;
  /** Login identifier - `<local>@center.{admin,assistant}.com`. */
  email: string;
  role: Role;
  /**
   ...
   * the web app to gate UI. Optional so an older client that ignores it keeps
...
/**
 * Credentials. Every account signs in with its email; ownership and role are
 * resolved server-side from the account, not from the address.
 */
