...
// Source: Unicode emoji-test.txt 15.1 (https://unicode.org/Public/emoji/15.1/emoji-test.txt),
// every fully-qualified emoji, skin-tone and gender variants included.
// Regenerate by re-parsing that file: keep only `fully-qualified` rows, drop the
// Component group, and emit [glyph, english name] per official group.

/** One emoji: its glyph, and its official English name (what search matches). */
export type Emoji = readonly [glyph: string, name: string];

export interface EmojiGroup {
  /** Arabic label shown on the tab. */
  label: string;
  /** A representative glyph, used as the tab icon. */
  icon: string;
...
/** Total glyphs across every group. */
export const EMOJI_COUNT = 3773;
